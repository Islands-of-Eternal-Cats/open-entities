...
use crate::entity_components::{EntityComponents, merge_components};
...
fn resolve_template(
    ...
    let entry = raw
        .get(name)
        .ok_or_else(|| ImportError::UnknownTemplateParent {
...
",
    ...
    fn import_error_template_cycle_display() {
        let err = ImportError::TemplateCycle {
            chain: vec!["scout".to_owned(), "unit".to_owned(), "scout".to_owned()],
        };
        assert_eq!(
            err.to_string(),
...
";
    ...
    fn spawn_entity_unknown_template() {
        ...
        load_fixture(&mut api);
        let err = api
            .spawn_entity("nope", EntityComponents::default())
            .unwrap_err();
    ...
    fn spawn_entity_base() {
        ...
        load_fixture(&mut api);
        let entity = api
            .spawn_entity("base", EntityComponents::default())
            .expect("spawn base")
            .to_entity()
            .expect("live entity");
        let world = api.core_mut().world_mut();
    ...
    fn spawn_entity_marker() {
        ...
        load_fixture(&mut api);
        let entity = api
            .spawn_entity("marker", EntityComponents::default())
            .expect("spawn marker")
            .to_entity()
            .expect("live entity");
        let world = api.core_mut().world_mut();
...
";
    ...
    fn spawn_entity_twice_same_name() {
        ...
        assert_eq!(
            world.get::<EntityType>(e1).map(|t| t.0.as_str()),
            Some("scout")
        );
        assert_eq!(
            world.get::<EntityType>(e2).map(|t| t.0.as_str()),
            Some("scout")
        );
    }
    ...
    fn load_templates_yaml_replaces() {
        ...
        let entity = api
            .spawn_entity("b", EntityComponents::default())
            .expect("only B remains")
            .to_entity()
            .expect("live entity");
        let world = api.core_mut().world_mut();
...
";
        ...
        api.load_templates_yaml(yaml).expect("load");
        let entity = api
            .spawn_entity("c", EntityComponents::default())
            .expect("spawn c")
            .to_entity()
            .expect("live entity");
        let world = api.core_mut().world_mut();
...
";
        ...
        let unit = api
            .spawn_entity("unit", EntityComponents::default())
            .expect("spawn unit")
            .to_entity()
            .expect("live entity");
        let clone = api
            .spawn_entity("clone", EntityComponents::default())
            .expect("spawn clone")
            .to_entity()
            .expect("live entity");
        let world = api.core_mut().world_mut();
...
";
        ...
        api.load_templates_yaml(yaml).expect("load");
        let entity = api
            .spawn_entity("heavy_tank", EntityComponents::default())
            .expect("spawn")
            .to_entity()
            .expect("live entity");
        let world = api.core_mut().world_mut();
...
";
        ...
        assert_eq!(
            w1.get::<Faction>(e1).map(|f| f.0),
            w2.get::<Faction>(e2).map(|f| f.0)
        );
        assert_eq!(
...
";
        ...
        api.load_templates_yaml(yaml).expect("load");
        let entity = api
            .spawn_entity("hybrid", EntityComponents::default())
            .expect("spawn")
            .to_entity()
            .expect("live entity");
        let world = api.core_mut().world_mut();
...
";
        ...
        let e1 = api_with
            .spawn_entity("bare", EntityComponents::default())
            .expect("spawn with")
            .to_entity()
            .expect("live entity");
        let e2 = api_without
            .spawn_entity("bare", EntityComponents::default())
            .expect("spawn without")
            .to_entity()
            .expect("live entity");
...
";
    ...
    fn load_failed_resolve_keeps_previous() {
        ...
        let err = api
            .load_templates_yaml("entities:\n  bad:\n    template: missing\n")
            .unwrap_err();
        assert!(matches!(err, ImportError::UnknownTemplateParent { .. }));
